...
export default function ProductDetail() {
  ...
  const { user, userData, setUserData, openAuthModal } = useStore();
  ...
  const handleGetProduct = async () => {
    hapticFeedback('medium');
    if (!user) {
      openAuthModal('Sign in to purchase this product.');
      return;
    }
  ...
  const toggleWishlist = async () => {
    ...
    if (!user) {
      openAuthModal('Sign in to add this to your wishlist.');
      return;
    }
